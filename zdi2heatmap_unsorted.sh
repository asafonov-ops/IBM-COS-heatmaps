...
for line in fileinput.input():
        if p.match (line.strip()):
            m=p.match (line.strip())
        if u.match (line.strip()):
            print(m.group(),line.strip())" |  awk '{ print $3 }' | sort -u  | awk 'BEGIN {i=1} { printf "disk%d ", i++ }' )
...
p=re.compile(r'^([a-z0-9]+\.)+([a-z]+|xn\-\-[a-z0-9]+)\.?')
u=re.compile('^[a-f0-9]{8}-?[a-f0-9]{4}-?4[a-f0-9]{3}-?[89ab][a-f0-9]{3}-?[a-f0-9]{12}', re.I)
d=re.compile('^Disk Id.?')

i=1

for line in fileinput.input():
        if d.match (line.strip()):
            i=1
        if p.match (line.strip()):
            m=p.match (line.strip())
        if u.match (line.strip()):
            print(i,m.group(),line.strip())
            i=i+1
            " |   awk '{ print $2",""disk"$1","$15" "}' | sed -e 's/%//')
